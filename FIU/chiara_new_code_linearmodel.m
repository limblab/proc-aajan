...
for s = 1:n_folds
    ...
    ts1 = zeros(splt1,1);
    ts2 = ones(splt2,1);
    idxTrain = logical(cat(1,tr1,tr2));%training(cross_val,s);%
    idxTest = logical(cat(1,ts1,ts2));%test(cross_val,s);%

    yfit_tr=zeros(splt1,size(mod_neu,2));%zeros(cross_val.TrainSize(s),size(mod_neu,2));
    yfit_ts=zeros(splt2,size(mod_neu,2));%zeros(cross_val.TestSize(s),size(mod_neu,2));
    ...
    for neu_n=1: size(mod_neu,2)
        ...
        R2_tr = aajan_rsquared(mod_neu(idxTrain,neu_n)', yfit_tr(:,neu_n)');
